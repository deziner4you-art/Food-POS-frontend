...
export default function MenuManager() {
 ...
 const cols = [
 { key: 'name', label: 'Menu Collection', render: (m: any) => <span className="font-bold text-stitch-ink text-base">{m.name}</span> },
 {
 ...
 <span key={s.id} className="bg-stitch-card px-2 py-1 rounded text-xs text-stitch-ink">{s.name}</span>
 )) : <span className="text-stitch-muted italic text-xs">None</span>}
 </div>
 )
 },
 {
 key: 'category_count',
 label: 'Categories',
 render: (m: any) => <span className="text-stitch-ink font-mono">{m.categories?.length || 0}</span>
 },
 {
 key: 'actions',
 label: 'Actions',
 ...
 <button onClick={() => handleDuplicateMenu(m.id)} className="text-stitch-accent hover:text-stitch-accent-hover flex items-center gap-1" title="Duplicate Menu">
 <Plus size={16}/> Copy
 </button>
 <button onClick={() => { setMenuForm({ id: m.id, name: m.name, store_ids: m.stores.map((s:any)=>s.id) }); setShowMenuModal(true); }} className="text-stitch-muted hover:text-stitch-ink"><Edit size={16}/></button>
 <button onClick={() => handleDeleteMenu(m.id)} className="text-stitch-danger hover:text-stitch-danger/80"><Trash2 size={16}/></button>
 </div>
 )
 }
 ];
 ...
 return [
 { key: 'name', label: 'Group Name', render: (g: any) => <span className="font-bold text-stitch-ink text-base">{g.name}</span> },
 {
 key: 'status',
 label: 'Status',
 render: (g: any) => (
 <span className={`px-2 py-0.5 rounded text-xs font-bold ${g.is_required ? 'bg-stitch-accent/20 text-stitch-accent' : 'bg-stitch-card text-stitch-ink'}`}>
 ...
 sortable: false,
 render: (g: any) => <span className="font-mono text-xs text-stitch-muted">Min: {g.min_selection} • Max: {g.max_selection}</span>
 },
 {
 key: 'choices',
 ...
 <span className="text-[10px] font-bold text-stitch-muted uppercase tracking-wider">Items</span>
 <button
 onClick={() => { setModifierForm({ id: 0, modifier_group_id: g.id, name: '', additional_price: 0 }); setShowModifierModal(true); }}
 className="text-[10px] text-stitch-accent hover:underline font-bold flex items-center gap-0.5"
 >
 <Plus size={10} /> Add Item
 </button>
 </div>
 {g.modifiers?.map((m: any) => (
 <div key={m.id} className="flex justify-between items-center text-xs bg-stitch-card/60 px-2 py-1 rounded border border-stitch-border">
 <span className="text-stitch-ink">{m.name}</span>
 <div className="flex items-center gap-2">
 <span className="text-stitch-success font-mono text-[10px]">+Rs. {m.additional_price}</span>
 <button onClick={() => handleDeleteModifier(m.id)} className="text-stitch-danger hover:text-stitch-danger/80"><Trash2 size={12}/></button>
 </div>
 </div>
 ))}
 {(!g.modifiers || g.modifiers.length === 0) && (
 <span className="text-[10px] text-stitch-muted italic">No choices</span>
 )}
 </div>
 )
 },
 ...
 render: (group: any) => (
 <div className="flex justify-end gap-3 items-center">
 <button onClick={() => { setGroupForm({ id: group.id, name: group.name, is_required: group.is_required, min_selection: group.min_selection, max_selection: group.max_selection }); setShowGroupModal(true); }} className="text-stitch-muted hover:text-stitch-ink"><Edit size={16}/></button>
 <button onClick={() => handleDeleteGroup(group.id)} className="text-stitch-danger hover:text-stitch-danger/80"><Trash2 size={16}/></button>
 </div>
 )
 }
 ];
 ...
 return [
 { key: 'name', label: 'Rule Name', render: (r: any) => <span className="font-bold text-stitch-ink text-base">{r.name}</span> },
 {
 key: 'type',
 label: 'Rule Type',
 render: (r: any) => <span className="text-stitch-accent font-bold text-sm">{r.type}</span>
 },
 {
 key: 'hours',
 label: 'Active Hours',
 sortable: false,
 render: (r: any) => r.type !== 'ALWAYS' ? <span className="font-mono text-stitch-ink text-xs">{r.start_time} - {r.end_time}</span> : <span className="text-stitch-muted italic text-xs">-</span>
 },
 {
 key: 'days',
 label: 'Active Days',
 sortable: false,
 render: (r: any) => r.type !== 'ALWAYS' ? <span className="text-stitch-ink text-xs">{r.days}</span> : <span className="text-stitch-muted italic text-xs">-</span>
 },
 {
 key: 'actions',
 label: 'Actions',
 ...
 render: (rule: any) => (
 <div className="flex justify-end gap-3 items-center">
 <button onClick={() => { setRuleForm({ id: rule.id, name: rule.name, type: rule.type, start_time: rule.start_time || '09:00', end_time: rule.end_time || '23:00', days: rule.days || 'Mon,Tue,Wed,Thu,Fri,Sat,Sun' }); setShowRuleModal(true); }} className="text-stitch-muted hover:text-stitch-ink"><Edit size={16}/></button>
 <button onClick={() => handleDeleteRule(rule.id)} className="text-stitch-danger hover:text-stitch-danger/80"><Trash2 size={16}/></button>
 </div>
 )
 }
 ];
 ...
 width: '60px',
 render: (g: any) => (
 <div className="w-10 h-10 rounded-lg flex items-center justify-center text-stitch-ink text-xl font-bold" style={{ backgroundColor: g.color || '#3b82f6' }}>
 {g.name.charAt(0)}
 </div>
 )
 },
 ...
 render: (g: any) => (
 <div>
 <h4 className="font-bold text-stitch-ink text-base">{g.name}</h4>
 <span className={`text-[10px] px-1.5 py-0.5 rounded font-bold ${g.is_active ? 'bg-stitch-success/20 text-stitch-success' : 'bg-stitch-danger/20 text-stitch-danger'}`}>
 ...
 key: 'description',
 label: 'Description',
 render: (g: any) => <span className="text-sm text-stitch-muted max-w-xs block truncate">{g.description || '-'}</span>
 },
 {
 key: 'sort_order',
 label: 'Sort Order',
 render: (g: any) => <span className="font-mono text-stitch-ink">{g.sort_order}</span>
 },
 {
 key: 'actions',
 label: 'Actions',
 ...
 render: (g: any) => (
 <div className="flex justify-end gap-3 items-center">
 <button onClick={() => { setCategoryGroupForm({ ...g }); setShowCategoryGroupModal(true); }} className="text-stitch-muted hover:text-stitch-ink"><Edit size={18}/></button>
 <button onClick={() => handleDeleteCategoryGroup(g.id)} className="text-stitch-danger hover:text-stitch-danger/80"><Trash2 size={18}/></button>
 </div>
 )
 }
 ];
 ...
 label: 'Sort',
 width: '60px',
 render: (c: any) => <span className="font-mono text-stitch-muted">{c.sort_order ?? 0}</span>
 },
 {
 key: 'name',
 label: 'Category Name',
 render: (c: any) => (
 <div className="font-bold text-stitch-ink flex items-center gap-3">
 {c.image_url ? (
 <img src={`${BACKEND_URL}${c.image_url}`} alt={c.name} className="w-8 h-8 rounded object-cover border border-stitch-border" />
 ) : (
 <div className="w-8 h-8 rounded bg-stitch-card flex items-center justify-center text-[10px] text-stitch-muted">No Img</div>
 )}
 {c.name}
 </div>
 )
 ...
 return (
 <span 
 className="px-2 py-1 rounded-xl text-xs font-bold text-stitch-ink flex items-center w-max gap-1.5"
 style={{ backgroundColor: group.color || '#3b82f6' }}
 >
 <div className="w-4 h-4 rounded-full bg-stitch-surface/20 flex items-center justify-center text-[10px]">
 {group.name.charAt(0)}
 </div>
 {group.name}
 </span>
 ...
 return <span className="text-stitch-muted italic text-sm">No Group</span>;
 }
 },
 {
 key: 'is_active',
 label: 'Status',
 render: (c: any) => (
 <span className={`px-2.5 py-1 rounded-full text-xs font-bold ${c.is_active ? 'bg-stitch-success/20 text-stitch-success' : 'bg-stitch-danger/20 text-stitch-danger'}`}>
 ...
 key: 'menu',
 label: 'Menu',
 render: (c: any) => <span className="text-stitch-muted text-sm">{c.menu?.name || 'Unassigned'}</span>
 },
 {
 key: 'assigned_branches',
 label: 'Assigned Branches',
 ...
 <span key={s.id} className="bg-stitch-card px-2 py-1 rounded text-[10px] uppercase font-bold tracking-wider text-stitch-ink">{s.name}</span>
 )) : <span className="text-stitch-muted italic text-xs">None</span>}
 </div>
 )
 },
 ...
 <button onClick={() => handleDeleteCategory(c.id)} className="text-stitch-danger hover:text-stitch-danger/80 transition-colors">
 <Trash2 size={18} />
 </button>
 </div>
 )
 ...
 <img src={`${BACKEND_URL}${p.thumbnail_url || p.image_url}`} alt={p.name} className="w-10 h-10 rounded object-cover border border-stitch-border" />
 ) : (
 <div className="w-10 h-10 rounded bg-stitch-card border border-stitch-border flex items-center justify-center text-[10px] text-stitch-muted">No Img</div>
 )
 },
 {
 key: 'name',
 label: 'Product Name',
 render: (p: any) => <span className="font-bold text-stitch-ink">{p.name}</span>
 },
 { key: 'sku', label: 'SKU', render: (p: any) => <span className="font-mono text-stitch-muted text-xs">{p.sku || '-'}</span> },
 {
 key: 'category',
 label: 'Category',
 render: (p: any) => <span className="text-stitch-ink text-sm">{p.categories?.map((c:any) => c.name).join(', ') || '-'}</span>
 },
 {
 key: 'category_group',
 label: 'Category Group',
 ...
 return <span className="text-stitch-ink text-sm">{uniqueGroups.join(', ') || 'Ungrouped'}</span>;
 }
 },
 {
 key: 'menu',
 ...
 return <span className="text-stitch-ink text-sm">{uniqueMenus.join(', ') || '-'}</span>;
 }
 },
 {
 key: 'modifiers',
 ...
 return g ? <span key={g.id} className="bg-stitch-card px-1.5 py-0.5 rounded text-[10px]">{g.name}</span> : null;
 })}
 </div>
 )
 },
 ...
 <span className="inline-flex items-center gap-1 text-stitch-success text-xs font-bold bg-stitch-success/10 px-2 py-0.5 rounded border border-stitch-success/20">
 <CheckCircle2 size={12} /> {p.recipe.name}
 </span>
 ) : <span className="text-stitch-muted text-xs italic">No Recipe</span>
 },
 { key: 'kitchen_station', label: 'Kitchen Station', render: (p: any) => <span className="text-stitch-ink text-xs">{p.kitchen_station || 'Default'}</span> },
 { key: 'printer_group', label: 'Printer Group', render: (p: any) => <span className="text-stitch-ink text-xs">{p.printer_group || 'Default'}</span> },
 { key: 'kds_group', label: 'KDS Group', render: (p: any) => <span className="text-stitch-ink text-xs">{p.kds_group || 'Default'}</span> },
 {
 key: 'availability',
 label: 'Availability Rule',
 render: (p: any) => {
 const rule = availabilityRules?.find((r:any) => r.id === p.availability_rule_id);
 return <span className="text-stitch-ink text-xs">{rule ? rule.name : '-'}</span>;
 }
 },
 {
 key: 'price',
 label: 'Price',
 render: (p: any) => <span className="font-mono font-bold text-stitch-success">{p.variants?.length > 0 ? `${p.variants.length} Sizes` : `Rs. ${p.price}`}</span>
 },
 { key: 'cost', label: 'Cost', render: (p: any) => <span className="font-mono text-stitch-danger text-xs">Rs. {p.cost || 0}</span> },
 {
 key: 'margin',
 label: 'Margin',
 render: (p: any) => {
 if (p.variants?.length > 0) return <span className="text-stitch-muted">-</span>;
 const cost = p.cost || 0;
 const price = p.price || 0;
 if (price === 0) return <span className="text-stitch-muted">0%</span>;
 const marginPct = ((price - cost) / price * 100).toFixed(1);
 return <span className="font-mono text-stitch-success text-xs">{marginPct}%</span>;
 }
 },
 { key: 'tax_rate', label: 'Tax', render: (p: any) => <span className="font-mono text-stitch-ink text-xs">{p.tax_rate || 0}%</span> },
 {
 key: 'is_active',
 label: 'Status',
 render: (p: any) => (
 <span className={`px-2 py-0.5 rounded text-[11px] font-bold ${p.is_active ? 'bg-stitch-success/20 text-stitch-success' : 'bg-stitch-danger/20 text-stitch-danger'}`}>
 ...
 key: 'updated_at',
 label: 'Updated',
 render: (p: any) => <span className="text-stitch-muted text-[10px]">{new Date(p.updated_at).toLocaleDateString()}</span>
 },
 {
 key: 'actions',
 label: 'Actions',
 ...
 <button onClick={() => handleDeleteProduct(p.id)} className="text-stitch-danger hover:text-stitch-danger/80 transition-colors">
 <Trash2 size={18} />
 </button>
 </div>
 )
 ...
 className={`px-5 py-2.5 rounded-xl font-bold transition-all flex items-center gap-2 text-sm ${activeTab === 'MENUS' ? 'bg-stitch-accent text-stitch-accent-ink accent-glow' : 'bg-stitch-panel text-stitch-muted hover:bg-stitch-surface'}`}
 >
 <Store size={16} /> Menu Collections
 </button>
 <button
 onClick={() => setActiveTab('CATEGORIES')}
 className={`px-5 py-2.5 rounded-xl font-bold transition-all flex items-center gap-2 text-sm ${activeTab === 'CATEGORIES' ? 'bg-stitch-accent text-stitch-accent-ink accent-glow' : 'bg-stitch-panel text-stitch-muted hover:bg-stitch-surface'}`}
 >
 <ListTree size={16} /> Categories
 </button>
 <button
 onClick={() => setActiveTab('CATEGORY_GROUPS')}
 className={`px-5 py-2.5 rounded-xl font-bold transition-all flex items-center gap-2 text-sm ${activeTab === 'CATEGORY_GROUPS' ? 'bg-stitch-accent text-stitch-accent-ink accent-glow' : 'bg-stitch-panel text-stitch-muted hover:bg-stitch-surface'}`}
 >
 <Sliders size={16} /> Category Groups
 </button>
 <button
 onClick={() => setActiveTab('PRODUCTS')}
 className={`px-5 py-2.5 rounded-xl font-bold transition-all flex items-center gap-2 text-sm ${activeTab === 'PRODUCTS' ? 'bg-stitch-accent text-stitch-accent-ink accent-glow' : 'bg-stitch-panel text-stitch-muted hover:bg-stitch-surface'}`}
 >
 <Utensils size={16} /> Menu Products
 </button>
 <button
 onClick={() => setActiveTab('MODIFIERS')}
 className={`px-5 py-2.5 rounded-xl font-bold transition-all flex items-center gap-2 text-sm ${activeTab === 'MODIFIERS' ? 'bg-stitch-accent text-stitch-accent-ink accent-glow' : 'bg-stitch-panel text-stitch-muted hover:bg-stitch-surface'}`}
 >
 <Sliders size={16} /> Modifier Groups
 </button>
 <button
 onClick={() => setActiveTab('AVAILABILITY')}
 className={`px-5 py-2.5 rounded-xl font-bold transition-all flex items-center gap-2 text-sm ${activeTab === 'AVAILABILITY' ? 'bg-stitch-accent text-stitch-accent-ink accent-glow' : 'bg-stitch-panel text-stitch-muted hover:bg-stitch-surface'}`}
 >
 <Clock size={16} /> Availability Rules
 </button>
 <button
 onClick={() => setActiveTab('PRODUCT_REQUESTS')}
 className={`px-5 py-2.5 rounded-xl font-bold transition-all flex items-center gap-2 text-sm ${activeTab === 'PRODUCT_REQUESTS' ? 'bg-stitch-accent text-stitch-accent-ink accent-glow' : 'bg-stitch-panel text-stitch-muted hover:bg-stitch-surface'}`}
 >
 <ClipboardList size={16} /> Product Requests
 </button>
 </div>

 <div className="flex-1 bg-stitch-panel border border-stitch-border rounded-xl overflow-hidden flex flex-col">

 {/* MENUS TAB */}
 {activeTab === 'MENUS' && (
 <div className="flex flex-col h-full">
 <div className="p-6 border-b border-stitch-border flex justify-between items-center bg-stitch-surface/50">
 <h3 className="text-xl font-bold text-stitch-ink flex items-center gap-2">
 <Store className="text-stitch-accent" /> Menu Collections (Branch Wise)
 </h3>
 <button
 ...
 <div className="flex flex-col h-full">
 <div className="p-4 border-b border-stitch-border flex justify-between items-center bg-stitch-surface/50">
 <h3 className="text-xl font-bold text-stitch-ink flex items-center gap-2">
 <Sliders className="text-stitch-accent" /> Category Groups
 </h3>
 <button
 onClick={() => {
 ...
 <div className="p-4 border-b border-stitch-border flex justify-between items-center bg-stitch-surface/50">
 <h3 className="text-xl font-bold text-stitch-ink flex items-center gap-2">
 <ListTree className="text-stitch-accent" /> Menu Categories
 </h3>
 <div className="flex items-center gap-3">
 <select
 value={categoryFilterGroupId}
 onChange={e => setCategoryFilterGroupId(parseInt(e.target.value) || 0)}
 className="bg-stitch-card border border-stitch-border rounded-xl p-1.5 text-stitch-ink text-sm focus:outline-none focus:border-stitch-accent w-64"
 >
 ...
 <div className="bg-stitch-surface border-b border-stitch-border p-3 flex flex-col gap-3">
 <div className="flex items-center justify-between">
 <div className="flex items-center gap-4">
 <span className="text-sm font-bold text-stitch-ink px-2">Total Categories: {categories.length}</span>
 {selectedCategories.length > 0 && (
 <div className="flex items-center gap-2">
 <button
 onClick={handleBulkDeleteCategories}
 className="flex items-center gap-2 bg-stitch-danger/20 hover:bg-stitch-danger/30 text-stitch-danger border border-stitch-danger/50 px-3 py-1.5 rounded-xl text-xs font-bold transition-colors"
 >
 <Trash2 size={14} /> Delete Selected ({selectedCategories.length})
 </button>
 <div className="flex items-center gap-1 border-l border-stitch-border pl-2 ml-2">
 <select
 value={bulkCategoryGroupId}
 onChange={e => setBulkCategoryGroupId(parseInt(e.target.value) || 0)}
 className="bg-stitch-card border border-stitch-border rounded-xl p-1.5 text-stitch-ink text-xs focus:outline-none focus:border-stitch-accent"
 >
 ...
 className="bg-stitch-accent hover:bg-stitch-accent-hover text-stitch-accent-ink px-3 py-1.5 rounded-xl text-xs font-bold transition-colors"
 >
 Apply
 </button>
 )}
 ...
 <div className="p-4 border-b border-stitch-border flex justify-between items-center bg-stitch-surface/50">
 <h3
 className="text-xl font-bold text-stitch-ink flex items-center gap-2 cursor-pointer select-none"
 onClick={() => setIsProductEngineExpanded(!isProductEngineExpanded)}
 >
 <Utensils className="text-stitch-accent" /> Menu Products Engine
 <span className="text-stitch-muted text-sm ml-2">{isProductEngineExpanded ? '▼' : '▶'}</span>
 </h3>
 ...
 className="flex items-center gap-2 bg-stitch-accent hover:bg-stitch-accent-hover text-stitch-accent-ink px-4 py-2 rounded-lg font-bold text-sm transition-colors accent-glow-hover"
 >
 <Plus size={16} /> Create Product
 </button>
 )}
 <button onClick={handleExportProductsCsv} className="flex items-center gap-2 bg-stitch-surface hover:bg-stitch-card text-stitch-ink px-4 py-2 rounded-lg font-bold text-sm transition-colors">
 <FileDown size={16} /> Export CSV
 </button>
 <label className="flex items-center gap-2 bg-stitch-surface hover:bg-stitch-card text-stitch-ink px-4 py-2 rounded-lg font-bold text-sm transition-colors cursor-pointer">
 ...
 {/* Form */}
 {isProductEngineExpanded && (
 <div className="bg-stitch-surface border-b border-stitch-border p-4 overflow-y-auto max-h-[360px]">
 <form onSubmit={handleProductSubmit} className="flex flex-col gap-4">
 <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
 <div>
 <label className="block text-xs font-bold text-stitch-muted mb-1">Product Name *</label>
 <input
 required type="text" placeholder="e.g. Zinger Burger"
 value={productForm.name} onChange={e => setProductForm({...productForm, name: e.target.value})}
 className="w-full bg-stitch-card border border-stitch-border rounded-xl p-2 text-stitch-ink text-sm focus:outline-none focus:border-stitch-accent"
 />
 </div>
 <div>
 <label className="block text-xs font-bold text-stitch-muted mb-1">Selling Price (Rs.) *</label>
 <input
 ...
 className="w-full bg-stitch-card border border-stitch-border rounded-xl p-2 text-stitch-success font-mono font-bold text-sm focus:outline-none focus:border-stitch-accent disabled:opacity-50"
 />
 </div>
 <div>
 <label className="block text-xs font-bold text-stitch-muted mb-1">SKU</label>
 <input
 ...
 <div>
 <label className="block text-xs font-bold text-stitch-muted mb-1">Barcode</label>
 <input
 ...
 <div>
 <label className="block text-xs font-bold text-stitch-muted mb-1">Product Image</label>
 {isEditingProduct && productForm.id ? (
 <div className="flex gap-3 items-center">
 <label className="bg-stitch-surface border border-stitch-border rounded-lg p-3 text-stitch-ink text-sm cursor-pointer hover:bg-stitch-card whitespace-nowrap">
 ...
 <>
 <img src={`${BACKEND_URL}${productForm.thumbnail_url || productForm.image_url}`} alt="Preview" className="w-12 h-12 rounded-lg object-cover border border-stitch-border" />
 <button type="button" onClick={handleProductImageRemove} className="text-stitch-danger text-xs font-bold hover:underline">Remove</button>
 </>
 )}
 </div>
 ) : (
 <p className="text-xs text-stitch-muted italic">Save the product first, then a Product Image option will appear here.</p>
 )}
 </div>

 <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
 <div className="relative" ref={categoryDropdownRef}>
 <label className="block text-xs font-bold text-stitch-muted mb-1">Categories *</label>
 <div
 onClick={() => setIsCategoryDropdownOpen(!isCategoryDropdownOpen)}
 className="w-full bg-stitch-card border border-stitch-border rounded-xl p-2 text-stitch-ink text-sm cursor-pointer flex justify-between items-center h-[38px]"
 >
 ...
 <div className="absolute top-[60px] left-0 w-full bg-stitch-card border border-stitch-border rounded-xl p-2 text-stitch-ink text-sm max-h-48 overflow-y-auto flex flex-col gap-1 z-50 ">
 {categories.map(c => (
 <label key={c.id} className="flex items-center gap-2 cursor-pointer hover:bg-stitch-surface p-1.5 rounded">
 <input
 type="checkbox"
 ...
 className="accent-stitch-accent w-4 h-4 cursor-pointer"
 />
 ...
 <div>
 <label className="block text-xs font-bold text-stitch-muted mb-1">Recipe Mapping</label>
 <select
 value={productForm.recipe_id} onChange={e => setProductForm({...productForm, recipe_id: parseInt(e.target.value)})}
 className="w-full bg-stitch-card border border-stitch-border rounded-xl p-2 text-stitch-ink text-sm focus:outline-none focus:border-stitch-accent h-[38px]"
 >
 ...
 <div>
 <label className="block text-xs font-bold text-stitch-muted mb-1">Availability Rule</label>
 <select
 ...
 <div>
 <label className="block text-xs font-bold text-stitch-muted mb-1">Tax Rate (%)</label>
 <input
 ...
 <div className="grid grid-cols-1 md:grid-cols-4 gap-3 bg-stitch-card/50 p-3 rounded-lg border border-stitch-border">
 <div>
 <label className="block text-xs font-bold text-stitch-muted mb-1">Kitchen Station</label>
 <input
 ...
 className="w-full bg-stitch-card border border-stitch-border rounded-xl p-2 text-stitch-ink text-sm"
 />
 </div>
 <div>
 <label className="block text-xs font-bold text-stitch-muted mb-1">Printer Group</label>
 <input
 ...
 <div>
 <label className="block text-xs font-bold text-stitch-muted mb-1">KDS Display Group</label>
 <input
 ...
 <label className="block text-xs font-bold text-stitch-muted mb-1">Modifier Groups</label>
 <div
 ...
 <label key={mg.id} className="flex items-center gap-2 cursor-pointer hover:bg-stitch-surface p-1.5 rounded">
 <input
 type="checkbox"
 ...
 {/* Variants Section */}
 <div className="p-3 bg-stitch-card/50 border border-stitch-border rounded-lg">
 ...
 className="w-4 h-4 accent-stitch-accent cursor-pointer"
 id="hasVariantsToggle"
 />
 <label htmlFor="hasVariantsToggle" className="text-sm font-bold text-stitch-ink cursor-pointer">Product Variants (e.g. Small, Medium, Large)</label>
 </div>
 <div className="flex items-center gap-3">
 <label className="flex items-center gap-2 text-xs font-bold text-stitch-ink">
 <input
 type="checkbox"
 ...
 className="bg-stitch-card border border-stitch-border rounded-xl p-1.5 text-stitch-ink text-xs flex-1"
 />
 <input
 ...
 className="bg-stitch-card border border-stitch-border rounded-xl p-1.5 text-stitch-success font-mono font-bold text-xs w-24"
 />
 <input
 ...
 className="bg-stitch-card border border-stitch-border rounded-xl p-1.5 text-stitch-ink text-xs w-24"
 />
 <select
 ...
 className="bg-stitch-card border border-stitch-border rounded-xl p-1.5 text-stitch-ink text-xs w-36"
 >
 <option value={0}>Standard Recipe</option>
 {recipes.map(r => <option key={r.id} value={r.id}>{r.name}</option>)}
 </select>
 <button type="button" onClick={() => { const nv = productForm.variants.filter((_: any, i: number) => i !== idx); setProductForm({...productForm, variants: nv}); }} className="text-stitch-danger p-1.5 hover:bg-stitch-danger/10 rounded-xl">
 <Trash2 size={14} />
 </button>
 </div>
 ))}
 <button type="button" onClick={() => setProductForm({...productForm, variants: [...productForm.variants, {name: '', price: 0, cost: 0, sku: '', barcode: '', recipe_id: 0}]})} className="text-xs text-stitch-accent flex items-center gap-1 mt-1 font-bold w-max hover:underline">
 ...
 <div className="flex justify-end gap-3 items-center">
 {isEditingProduct && (
 <button type="button" onClick={() => { setProductForm(initialProductForm); setIsEditingProduct(false); }} className="px-4 py-2 text-xs font-bold text-stitch-muted hover:text-stitch-ink">Cancel</button>
 )}
 <button type="submit" className="bg-stitch-accent hover:bg-stitch-accent-hover text-stitch-accent-ink rounded-xl px-6 py-2 font-bold text-sm transition-colors flex items-center gap-2 accent-glow-hover">
 ...
 <div className="flex items-center justify-between">
 <div className="flex items-center gap-4">
 <span className="text-sm font-bold text-stitch-ink px-2">Total Products: {products.length}</span>
 ...
 <Search size={16} className="absolute left-3 top-1/2 transform -translate-y-1/2 text-stitch-muted" />
 <input
 type="text"
 placeholder="Search products by name, SKU..."
 value={productSearch}
 onChange={e => setProductSearch(e.target.value)}
 className="w-full pl-9 pr-3 py-1.5 bg-stitch-card border border-stitch-border rounded-xl text-stitch-ink text-sm focus:outline-none focus:border-stitch-accent"
 />
 </div>
 <select
 value={productFilterMenuId}
 onChange={e => setProductFilterMenuId(parseInt(e.target.value) || 0)}
 className="bg-stitch-card border border-stitch-border rounded-xl p-1.5 text-stitch-ink text-sm focus:outline-none focus:border-stitch-accent w-48"
 >
 ...
 className="bg-stitch-card border border-stitch-border rounded-xl p-1.5 text-stitch-ink text-sm focus:outline-none focus:border-stitch-accent w-32"
 >
 ...
 <div className="p-4 border-b border-stitch-border flex justify-between items-center bg-stitch-surface/50">
 <h3 className="text-xl font-bold text-stitch-ink flex items-center gap-2">
 <Sliders className="text-stitch-accent" /> Modifier Groups & Options
 </h3>
 <button
 ...
 <div className="p-4 border-b border-stitch-border flex justify-between items-center bg-stitch-surface/50">
 <h3 className="text-xl font-bold text-stitch-ink flex items-center gap-2">
 <Clock className="text-stitch-accent" /> Menu Availability Rules
 </h3>
 <button
 ...
 <div className="bg-stitch-panel border border-stitch-border rounded-2xl p-6 w-full max-w-sm animate-scale-up">
 <h3 className="text-xl font-bold text-stitch-ink mb-4">{menuForm.id ? 'Edit Menu Collection' : 'Create Menu Collection'}</h3>
 <form onSubmit={handleMenuSubmit}>
 <label className="block text-xs font-bold text-stitch-muted mb-1">Collection Name</label>
 <input
 required type="text" value={menuForm.name} onChange={e => setMenuForm({...menuForm, name: e.target.value})}
 className="w-full bg-stitch-surface border border-stitch-border rounded-lg p-3 text-stitch-ink focus:outline-none focus:border-stitch-accent mb-4"
 placeholder="e.g. Main Branch Menu"
 />
 <label className="block text-xs font-bold text-stitch-muted mb-2">Assign to Branches:</label>
 <div className="flex flex-col gap-2 mb-6 max-h-40 overflow-y-auto">
 {stores.map(s => (
 <label key={s.id} className="flex items-center gap-2 cursor-pointer text-sm text-stitch-ink">
 <input
 type="checkbox"
 ...
 <button type="button" onClick={() => setShowMenuModal(false)} className="flex-1 py-3 rounded-lg font-bold text-stitch-muted bg-stitch-surface hover:bg-stitch-card">Cancel</button>
 <button type="submit" className="flex-1 py-3 rounded-lg font-bold text-stitch-accent-ink bg-stitch-accent hover:bg-stitch-accent-hover accent-glow-hover">Save</button>
 </div>
 </form>
 </div>
 </div>
 ...
 <div className="fixed inset-0 bg-stitch-bg/80 backdrop-blur-sm flex items-center justify-center z-50">
 <div className="bg-stitch-panel border border-stitch-border rounded-2xl p-6 w-full max-w-lg animate-scale-up max-h-[90vh] overflow-y-auto">
 <h3 className="text-xl font-bold text-stitch-ink mb-4">{categoryGroupForm.id ? 'Edit Category Group' : 'Create Category Group'}</h3>
 <form onSubmit={handleCategoryGroupSubmit}>
 <div className="grid grid-cols-2 gap-4 mb-4">
 <div className="col-span-2">
 <label className="block text-xs font-bold text-stitch-muted mb-1">Group Name *</label>
 <input
 ...
 <div>
 <label className="block text-xs font-bold text-stitch-muted mb-1">Sort Order</label>
 <input
 ...
 className="w-full bg-stitch-surface border border-stitch-border rounded-lg p-2.5 text-stitch-ink"
 />
 </div>
 <div>
 <label className="block text-xs font-bold text-stitch-muted mb-1">Color (Hex)</label>
 <input
 ...
 <div className="col-span-2">
 <label className="block text-xs font-bold text-stitch-muted mb-1">Description</label>
 <textarea
 ...
 <div className="mb-4">
 <label className="flex items-center gap-2 cursor-pointer text-sm text-stitch-ink font-bold bg-stitch-surface p-3 rounded-lg border border-stitch-border w-max">
 <input
 type="checkbox"
 checked={categoryGroupForm.is_active}
 onChange={e => setCategoryGroupForm({...categoryGroupForm, is_active: e.target.checked})}
 className="accent-stitch-accent w-5 h-5 cursor-pointer"
 />
 Active Status
 </label>
 </div>
 ...
 <div className="mb-6">
 <label className="block text-xs font-bold text-stitch-muted mb-2">Channel Visibility</label>
 <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
 {Object.keys(categoryGroupForm.channel_visibility).map(channel => (
 <label key={channel} className="flex items-center gap-2 cursor-pointer text-xs text-stitch-ink">
 <input
 type="checkbox"
 ...
 <div className="flex gap-3">
 <button type="button" onClick={() => setShowCategoryGroupModal(false)} className="flex-1 py-3 rounded-lg font-bold text-stitch-muted bg-stitch-surface hover:bg-stitch-card">Cancel</button>
 <button type="submit" className="flex-1 py-3 rounded-lg font-bold text-stitch-accent-ink bg-stitch-accent hover:bg-stitch-accent-hover accent-glow-hover">Save</button>
 </div>
 </form>
 </div>
 ...
 {showCategoryModal && (
 <div className="fixed inset-0 bg-stitch-bg/80 backdrop-blur-sm flex items-center justify-center z-50">
 <div className="bg-stitch-panel border border-stitch-border rounded-2xl p-6 w-full max-w-md animate-scale-up">
 <h3 className="text-xl font-bold text-stitch-ink mb-4">{categoryForm.id ? 'Edit Category' : 'Add Category'}</h3>
 <form onSubmit={handleCategorySubmit}>
 <label className="block text-xs font-bold text-stitch-muted mb-1">Category Name *</label>
 <input
 ...
 <div>
 <label className="block text-xs font-bold text-stitch-muted mb-1">Category Group</label>
 <select
 ...
 <label className="block text-xs font-bold text-stitch-muted mb-1">Category Image</label>
 <div className="flex gap-3 items-center mb-4">
 <label className="bg-stitch-surface border border-stitch-border rounded-lg p-3 text-stitch-ink text-sm cursor-pointer hover:bg-stitch-card flex-1 text-center whitespace-nowrap">
 ...
 {categoryForm.image_url && <img src={`${BACKEND_URL}${categoryForm.image_url}`} alt="Preview" className="w-12 h-12 rounded-lg object-cover border border-stitch-border" />}
 </div>
 <div className="flex gap-3">
 <button type="button" onClick={() => setShowCategoryModal(false)} className="flex-1 py-3 rounded-lg font-bold text-stitch-muted bg-stitch-surface hover:bg-stitch-card">Cancel</button>
 <button type="submit" className="flex-1 py-3 rounded-lg font-bold text-stitch-accent-ink bg-stitch-accent hover:bg-stitch-accent-hover accent-glow-hover">Save</button>
 </div>
 </form>
 </div>
 ...
 <div className="fixed inset-0 bg-stitch-bg/80 backdrop-blur-sm flex items-center justify-center z-50">
 <div className="bg-stitch-panel border border-stitch-border rounded-2xl p-6 w-full max-w-sm animate-scale-up">
 <h3 className="text-xl font-bold text-stitch-ink mb-4">{groupForm.id ? 'Edit Modifier Group' : 'Add Modifier Group'}</h3>
 ...
 className="accent-stitch-accent w-4 h-4 cursor-pointer" id="reqCheck"
 />
 <label htmlFor="reqCheck" className="text-sm font-bold text-stitch-ink cursor-pointer">Required Selection</label>
 </div>
 <div className="grid grid-cols-2 gap-3 mb-6">
 <div>
 <label className="block text-xs font-bold text-stitch-muted mb-1">Min Selection</label>
 <input
 ...
 <div>
 <label className="block text-xs font-bold text-stitch-muted mb-1">Max Selection</label>
 <input
 ...
 <div className="flex gap-3">
 <button type="button" onClick={() => setShowGroupModal(false)} className="flex-1 py-3 rounded-lg font-bold text-stitch-muted bg-stitch-surface hover:bg-stitch-card">Cancel</button>
 <button type="submit" className="flex-1 py-3 rounded-lg font-bold text-stitch-accent-ink bg-stitch-accent hover:bg-stitch-accent-hover accent-glow-hover">Save</button>
 </div>
 </form>
 </div>
 ...
 <div className="fixed inset-0 bg-stitch-bg/80 backdrop-blur-sm flex items-center justify-center z-50">
 <div className="bg-stitch-panel border border-stitch-border rounded-2xl p-6 w-full max-w-sm animate-scale-up">
 <h3 className="text-xl font-bold text-stitch-ink mb-4">Add Modifier Choice</h3>
 <form onSubmit={handleModifierSubmit}>
 <label className="block text-xs font-bold text-stitch-muted mb-1">Modifier Name *</label>
 <input
 required type="text" value={modifierForm.name} onChange={e => setModifierForm({...modifierForm, name: e.target.value})}
 placeholder="e.g. Extra Cheese"
 className="w-full bg-stitch-surface border border-stitch-border rounded-lg p-3 text-stitch-ink focus:outline-none focus:border-stitch-accent mb-4"
 />
 <label className="block text-xs font-bold text-stitch-muted mb-1">Additional Price (Rs.)</label>
 <input
 ...
 placeholder="50"
 className="w-full bg-stitch-surface border border-stitch-border rounded-lg p-3 text-stitch-success font-mono font-bold focus:outline-none focus:border-stitch-accent mb-6"
 />
 <div className="flex gap-3">
 <button type="button" onClick={() => setShowModifierModal(false)} className="flex-1 py-3 rounded-lg font-bold text-stitch-muted bg-stitch-surface hover:bg-stitch-card">Cancel</button>
 <button type="submit" className="flex-1 py-3 rounded-lg font-bold text-stitch-accent-ink bg-stitch-accent hover:bg-stitch-accent-hover accent-glow-hover">Save</button>
 </div>
 </form>
 </div>
 ...
 <div className="fixed inset-0 bg-stitch-bg/80 backdrop-blur-sm flex items-center justify-center z-50">
 <div className="bg-stitch-panel border border-stitch-border rounded-2xl p-6 w-full max-w-sm animate-scale-up">
 <h3 className="text-xl font-bold text-stitch-ink mb-4">{ruleForm.id ? 'Edit Rule' : 'Add Availability Rule'}</h3>
 <form onSubmit={handleRuleSubmit}>
 <label className="block text-xs font-bold text-stitch-muted mb-1">Rule Name</label>
 <input
 required type="text" value={ruleForm.name} onChange={e => setRuleForm({...ruleForm, name: e.target.value})}
 className="w-full bg-stitch-surface border border-stitch-border rounded-lg p-3 text-stitch-ink focus:outline-none focus:border-stitch-accent mb-4"
 />
 <label className="block text-xs font-bold text-stitch-muted mb-1">Type</label>
 <select
 value={ruleForm.type} onChange={e => setRuleForm({...ruleForm, type: e.target.value})}
 className="w-full bg-stitch-surface border border-stitch-border rounded-lg p-3 text-stitch-ink mb-4"
 >
 ...
 <div className="grid grid-cols-2 gap-3 mb-4">
 <div>
 <label className="block text-xs font-bold text-stitch-muted mb-1">Start Time</label>
 <input type="time" value={ruleForm.start_time} onChange={e => setRuleForm({...ruleForm, start_time: e.target.value})} className="w-full bg-stitch-surface border border-stitch-border rounded-lg p-2.5 text-stitch-ink" />
 </div>
 <div>
 <label className="block text-xs font-bold text-stitch-muted mb-1">End Time</label>
 <input type="time" value={ruleForm.end_time} onChange={e => setRuleForm({...ruleForm, end_time: e.target.value})} className="w-full bg-stitch-surface border border-stitch-border rounded-lg p-2.5 text-stitch-ink" />
 </div>
 </div>
 )}
 <div className="flex gap-3">
 <button type="button" onClick={() => setShowRuleModal(false)} className="flex-1 py-3 rounded-lg font-bold text-stitch-muted bg-stitch-surface hover:bg-stitch-card">Cancel</button>
 <button type="submit" className="flex-1 py-3 rounded-lg font-bold text-stitch-accent-ink bg-stitch-accent hover:bg-stitch-accent-hover accent-glow-hover">Save</button>
 </div>
 </form>
 </div>
 ...
 {/* CSV Preview Modal */}
 {csvPreviewData && (
 <div className="fixed inset-0 bg-stitch-bg/90 flex items-center justify-center p-4 z-50 animate-fade-in overflow-y-auto">
 <div className="bg-stitch-panel rounded-xl p-6 w-full max-w-5xl shadow-2xl border border-stitch-border my-8 flex flex-col max-h-[90vh]">
 <h2 className="text-xl font-bold text-stitch-ink mb-4 flex items-center gap-2">
 <FileUp className="text-stitch-accent" /> CSV Import Preview
 </h2>

 {csvErrors.length > 0 && (
 <div className="mb-4 bg-stitch-danger/10 border border-stitch-danger/30 rounded-lg p-4 max-h-40 overflow-y-auto">
 <h4 className="text-stitch-danger font-bold text-sm mb-2 flex items-center gap-2">
 <AlertCircle size={16} /> Validation Errors Found
 </h4>
 <ul className="list-disc pl-5 text-stitch-danger/90 text-xs space-y-1">
 ...
 <div className="flex-1 overflow-auto border border-stitch-border rounded-lg bg-stitch-surface">
 <table className="w-full text-left border-collapse text-sm whitespace-nowrap">
 <thead className="bg-stitch-panel text-xs uppercase font-bold text-stitch-muted sticky top-0 z-10">
 <tr>
 {csvPreviewData.headers.map((header, idx) => (
 <th key={idx} className="p-3 border-b border-stitch-border">{header}</th>
 ))}
 </tr>
 </thead>
 <tbody>
 {csvPreviewData.rows.map((row, rIdx) => (
 <tr key={rIdx} className="border-b border-stitch-border hover:bg-stitch-card/50">
 {csvPreviewData.headers.map((header, cIdx) => (
 <td key={cIdx} className="p-3 text-stitch-ink">
 {row[header]}
 </td>
 ))}
 </tr>
 ...
 <tr>
 <td colSpan={csvPreviewData.headers.length} className="p-4 text-center text-stitch-muted">
 ...
 <p className="text-stitch-muted text-xs mt-3 mb-6 italic">
 ...
 className="flex-1 py-3 rounded-lg font-bold text-stitch-muted bg-stitch-surface hover:bg-stitch-card border border-stitch-border"
 >
 Cancel
 </button>
 <button
 ...
 ${csvErrors.length > 0 ? 'bg-stitch-muted/30 text-stitch-ink cursor-not-allowed opacity-50' : 'bg-stitch-accent hover:bg-stitch-accent-hover text-stitch-accent-ink accent-glow-hover'}`}
 >
