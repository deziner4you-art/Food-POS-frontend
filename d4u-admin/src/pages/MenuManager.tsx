...
export default function MenuManager() {
  ...
  const [productForm, setProductForm] = useState({ id: 0, name: '', price: 0, category_id: 0, sku: '', image_url: '', assigned_store_ids: [] as number[] });
  ...
  const handleProductSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (productForm.category_id === 0) return setToastMessage('Please select a category');
    try {
      ...
      const payload = isEditingProduct 
        ? { name: productForm.name, price: productForm.price, category_id: productForm.category_id, sku: productForm.sku, image_url: productForm.image_url, assigned_store_ids: productForm.assigned_store_ids }
        : { store_id: 1, name: productForm.name, price: productForm.price, category_id: productForm.category_id, sku: productForm.sku, image_url: productForm.image_url, cost: 0, margin_pct: 100, status: 'APPROVED', assigned_store_ids: productForm.assigned_store_ids };
      ...
      if (res.ok) {
        setProductForm({ id: 0, name: '', price: 0, category_id: 0, sku: '', image_url: '', assigned_store_ids: [] });
  ...
  const handleDeleteMenu = (id: number) => {
    setConfirmDialog({
      message: 'Delete this menu collection?',
      onConfirm: async () => {
        try {
  ...
  const handleDuplicateMenu = (id: number) => {
    setConfirmDialog({
      message: 'Create a duplicate copy of this menu collection?',
      onConfirm: async () => {
        try {
          await fetch(`${BACKEND_URL}/catalog/menus/${id}/duplicate`, { method: 'POST' });
          fetchAll();
          setToastMessage('Menu duplicated successfully');
        } catch (e) { console.error(e); }
        setConfirmDialog(null);
      }
    });
    ...
    <div className="animate-fade-in flex flex-col h-[calc(100vh-160px)]">
            ...
            <div className="bg-slate-900 border-b border-slate-700 p-4 flex flex-col gap-4">
              <form onSubmit={handleProductSubmit} className="flex flex-col gap-3 w-full">
                <div className="flex items-end gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <input 
                      ...
                      className="w-full bg-[#1e293b] border border-[#334155] rounded-md p-2 text-[#4edea3] font-mono font-bold text-sm focus:outline-none focus:border-[#fbbf24]"
                    />
                  </div>
                  ...
                  <div className="flex-1">
                    ...
                    {isEditingProduct && (
                      <button type="button" onClick={() => { setProductForm({ id: 0, name: '', price: 0, category_id: 0, sku: '', image_url: '', assigned_store_ids: [] }); setIsEditingProduct(false); }} className="w-full mt-1 text-xs text-slate-400 hover:text-white">Cancel Edit</button>
                    )}
                  </div>
                </div>
                ...
                <div className="bg-[#1e293b] p-3 rounded-md border border-[#334155]">
                  <label className="block text-xs font-bold text-slate-400 mb-2">Assign to Specific Branches (Optional if Category is in an Assigned Menu):</label>
                  <div className="flex flex-wrap gap-4">
                    {stores.map(s => (
                      <label key={s.id} className="flex items-center gap-2 cursor-pointer text-sm text-white">
                        <input 
                          type="checkbox" 
                          checked={productForm.assigned_store_ids.includes(s.id)}
                          onChange={() => handleStoreToggle(s.id, productForm.assigned_store_ids, (ids) => setProductForm({...productForm, assigned_store_ids: ids}))}
                ...
                <tbody>
                  {products.map(p => (
                    <tr key={p.id} className="border-t border-slate-700/50 hover:bg-slate-700/20">
                      ...
                      <td className="p-4">
                        {p.assigned_stores?.length > 0 
                          ? p.assigned_stores.map((s:any) => s.name).join(', ') 
                          : <span className="text-slate-500 text-xs italic">Via Menu Rules</span>}
                      </td>
                      <td className="p-4 flex justify-end gap-3 items-center">
                        <button 
                          onClick={() => { setProductForm({ id: p.id, name: p.name, price: p.price, category_id: p.category_id, sku: p.sku || '', image_url: p.image_url || '', assigned_store_ids: p.assigned_stores.map((s:any)=>s.id) }); setIsEditingProduct(true); }} 
      ...
      {confirmDialog && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[60]">
          <div className="bg-slate-800 border border-slate-700 rounded-2xl p-6 w-full max-w-sm animate-scale-up text-center">
            <h3 className="text-xl font-bold text-white mb-6">{confirmDialog.message}</h3>
            <div className="flex gap-3">
              <button onClick={() => setConfirmDialog(null)} className="flex-1 py-3 rounded-lg font-bold text-slate-400 bg-slate-900 hover:bg-slate-700">Cancel</button>
              <button onClick={confirmDialog.onConfirm} className="flex-1 py-3 rounded-lg font-bold text-white bg-red-500 hover:bg-red-600">Yes, Delete</button>
            </div>
          </div>
        </div>
      )}
      ...
      {/* Toast Notification */}
      {toastMessage && (
        <div className="fixed bottom-6 right-6 bg-red-500 text-white px-6 py-3 rounded-lg shadow-lg font-bold animate-fade-in z-[60]">
          {toastMessage}
        </div>
      )}
