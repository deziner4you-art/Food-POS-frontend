...
**Status:** In Progress (NestJS Boots Successfully, Database Schema Synced)
**Goal:** Move away from local JSON files and fully transition to the `d4u-pos-backend` NestJS application.
**Action Plan:**
1. ~~Fix the `UnknownDependenciesException` for `AuthService/PrismaService` in the NestJS backend.~~ (Completed - App boots cleanly)
2. ~~Configure a local database (PostgreSQL/MySQL).~~ (Completed - PostgreSQL `d4u_pos_db` is connected)
3. ~~Generate Prisma client (`npx prisma generate`) and push schema (`npx prisma db push`).~~ (Completed - DB is in sync with Prisma)
4. Migrate all routes from the temporary Node.js bridge to the NestJS controllers (In Progress).
5. Create a data migration script to safely transfer active orders from `live_orders.json` into the new SQL database.
6. Transfer `Socket.io` events (`new_order`, `order_updated`, `gps_update`) to NestJS `app.gateway.ts`.
7. Shut down `order-bridge.cjs` and run NestJS on port 3001 permanently.


---

## PHASE 5: Multi-Store Scoping & Product Synchronization
**Status:** In Progress
**Goal:** Enable support for 40+ branches by ensuring online orders, inventory, and WebSocket events are strictly isolated per `store_id`.
**Action Plan:**
1. Update `schema.prisma` to include `store_id` in the `OnlineOrder` model.
2. Upgrade `app.gateway.ts` to use Socket.io Rooms (`store_1`, `store_2`) so POS clients only receive orders for their specific branch.
3. Update `online-orders.service.ts` to save and filter orders by `store_id`.
4. Update POS Client (`App.tsx`) to join its specific branch's WebSocket room on startup.
5. Update Customer Website to allow Branch Selection before placing an order.
