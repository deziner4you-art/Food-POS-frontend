...
const USERS = [
  { email: 'cashier@d4u.com',  password: '1234',  name: 'Ali Cashier',  role: 'Cashier' },
  { email: 'manager@d4u.com',  password: 'manager123', name: 'Sara Manager', role: 'Manager' },
  { email: 'admin@d4u.com',    password: 'admin',  name: 'Super Admin',  role: 'Admin'   },
  { email: 'chef@d4u.com',     password: '1234',  name: 'Head Chef',    role: 'Chef'    },
];

function LoginScreen({ onLogin }: { onLogin: (user: typeof USERS[0]) => void }) {
  const [email, setEmail]       = useState('');
  const [password, setPassword] = useState('');
  const [errors, setErrors]     = useState({ email: false, password: false, invalid: false });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (emailErr || passwordErr) { setErrors({ email: emailErr, password: passwordErr, invalid: false }); return; }
    ...
    setTimeout(() => {
      ...
      if (user) {
        ...
        setErrors({ email: false, password: false, invalid: true });
        setLoading(false);
      }
    }, 600);
  };
    ...
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(135deg, #1a0a00 0%, #3d1f00 40%, #1a0a00 100%)', fontFamily: "'Outfit', sans-serif", position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ background: 'white', borderRadius: '16px', padding: '40px 36px', width: '340px', boxShadow: '0 25px 60px rgba(0,0,0,0.5)', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0' }}>
        ...
        <form onSubmit={handleSubmit} style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: '14px' }}>
          ...
          <div style={{ position: 'relative' }}>
            <input
              ...
              placeholder="Email"
              value={email}
              onChange={e => { setEmail(e.target.value); setErrors(p => ({ ...p, email: false, invalid: false })); }}
              style={{ width: '100%', padding: '12px 40px 12px 14px', border: `1px solid ${errors.email ? '#ff4d4f' : '#ddd'}`, borderRadius: '8px', fontSize: '0.9rem', outline: 'none', color: '#333', boxSizing: 'border-box', background: errors.email ? '#fff2f0' : 'white' }}
            />
            {errors.email && (
        ...
        <p style={{ marginTop: '18px', fontSize: '0.7rem', color: '#aaa', textAlign: 'center', lineHeight: '1.5' }}>
          Demo: <strong>cashier@d4u.com</strong> / <strong>1234</strong>
        </p>
      </div>
...
function POSApp({ currentUser: _currentUser, dayStartTime }: { currentUser: typeof USERS[0]; dayStartTime: Date }) {
  ...
  useEffect(() => {
    const seedDB = async () => {
      try {
        ...
        const firstProduct = productsCount > 0 ? await db.products.toCollection().first() : null;
        const needsReseed = productsCount === 0 || !firstProduct || !firstProduct.desc;
        ...
        if (needsReseed) {
          ...
          await db.categories.bulkPut([
            { id: 1, store_id: 1, name: 'Burgers & Sandwiches' },
            { id: 2, store_id: 1, name: 'Pizzas' },
            { id: 3, store_id: 1, name: 'Cold Beverages' },
            { id: 4, store_id: 1, name: 'Desserts' }
          ]);

          await db.products.bulkPut([
            { id: 1, category_id: 1, name: 'Zinger Deluxe Burger', price: 450, desc: 'Crispy chicken fillet with cheese, lettuce and mayo', img: 'https://images.unsplash.com/photo-1568901346375-23c9450c58cd?auto=format&fit=crop&w=300&q=80' },
            { id: 2, category_id: 1, name: 'Grilled Chicken Sandwich', price: 380, desc: 'Juicy grilled breast slice with cheese and special sauce', img: 'https://images.unsplash.com/photo-1521305916504-4a1121188589?auto=format&fit=crop&w=300&q=80' },
            { id: 3, category_id: 2, name: 'Chicken Fajita Pizza (Large)', price: 1200, desc: 'Rich pizza loaded with chicken chunks, onions, peppers and cheese', img: 'https://images.unsplash.com/photo-1513104890138-7c749659a591?auto=format&fit=crop&w=300&q=80' },
            { id: 4, category_id: 2, name: 'Pepperoni Pizza (Medium)', price: 950, desc: 'Classic marinara sauce with pepperoni slices and mozzarella', img: 'https://images.unsplash.com/photo-1628840042765-356cda07504e?auto=format&fit=crop&w=300&q=80' },
            { id: 5, category_id: 3, name: 'Mint Margarita', price: 220, desc: 'Refreshing blender beverage with mint, lemon and syrup', img: 'https://images.unsplash.com/photo-1497534446932-c925b458314e?auto=format&fit=crop&w=300&q=80' },
            { id: 6, category_id: 3, name: 'Fresh Lime Soda', price: 150, desc: 'Sparkling cold beverage with lemon and soda', img: 'https://images.unsplash.com/photo-1513558161293-cdaf765ed2fd?auto=format&fit=crop&w=300&q=80' },
            { id: 7, category_id: 4, name: 'Chocolate Fudge Brownie', price: 280, desc: 'Rich, gooey fudge chocolate brownie topped with chocolate syrup', img: 'https://images.unsplash.com/photo-1606313564200-e75d5e30476c?auto=format&fit=crop&w=300&q=80' }
          ]);
        }
      } catch (err) {
        console.error('Failed to seed offline database:', err);
      }
    };
    seedDB();
  }, []);
  ...
  const handleRepeatOrder = (itemsStr: string) => {
    const items = itemsStr.split(',').map(s => s.trim());
    const newCartItems: any[] = [];

    const itemMap: { [key: string]: number } = {
      'Zinger Deluxe': 1,
      'Zinger Deluxe Burger': 1,
      'Mint Margarita': 5,
      'Chicken Fajita Pizza (Large)': 3,
      'Fajita Pizza': 3,
      'Large Pizza': 3,
      'Fresh Lime Soda': 6,
    ...
    const mockProducts: any[] = [
      { id: 1, name: 'Zinger Deluxe Burger', price: 450, desc: 'Crispy chicken fillet with cheese, lettuce and mayo', img: 'https://images.unsplash.com/photo-1568901346375-23c9450c58cd?auto=format&fit=crop&w=300&q=80' },
      { id: 2, name: 'Grilled Chicken Sandwich', price: 380, desc: 'Juicy grilled breast slice with cheese and special sauce', img: 'https://images.unsplash.com/photo-1521305916504-4a1121188589?auto=format&fit=crop&w=300&q=80' },
      { id: 3, name: 'Chicken Fajita Pizza (Large)', price: 1200, desc: 'Rich pizza loaded with chicken chunks, onions, peppers and cheese', img: 'https://images.unsplash.com/photo-1513104890138-7c749659a591?auto=format&fit=crop&w=300&q=80' },
      { id: 4, name: 'Pepperoni Pizza (Medium)', price: 950, desc: 'Classic marinara sauce with pepperoni slices and mozzarella', img: 'https://images.unsplash.com/photo-1628840042765-356cda07504e?auto=format&fit=crop&w=300&q=80' },
      { id: 5, name: 'Mint Margarita', price: 220, desc: 'Refreshing blender beverage with mint, lemon and syrup', img: 'https://images.unsplash.com/photo-1497534446932-c925b458314e?auto=format&fit=crop&w=300&q=80' },
      { id: 6, name: 'Fresh Lime Soda', price: 150, desc: 'Sparkling cold beverage with lemon and soda', img: 'https://images.unsplash.com/photo-1513558161293-cdaf765ed2fd?auto=format&fit=crop&w=300&q=80' },
      { id: 7, name: 'Chocolate Fudge Brownie', price: 280, desc: 'Rich, gooey fudge chocolate brownie topped with chocolate syrup', img: 'https://images.unsplash.com/photo-1606313564200-e75d5e30476c?auto=format&fit=crop&w=300&q=80' }
    ];
    ...
    items.forEach(item => {
      ...
      if (match) {
        ...
        let prodId = itemMap[name];
        if (!prodId) {
          const keys = Object.keys(itemMap);
          const foundKey = keys.find(k => name.toLowerCase().includes(k.toLowerCase()));
          if (foundKey) prodId = itemMap[foundKey];
        }
  ...
  const simulatedChats = [
    {
      ...
      name: 'Raza',
      isOld: true,
      message: '1 Large Pizza bhej dain usi address per.',
      history: 'Last time ordered 1x Chicken Fajita Pizza (Large)',
      repeatItems: '1x Chicken Fajita Pizza (Large)'
    },
    {
      id: 2,
      phone: '03459876543',
      name: 'Hassan',
      isOld: false,
      message: 'Zinger Deluxe Burger aur Fresh Lime Soda chahiye, Clifton Phase 6.',
      history: 'No previous history (New Customer)',
      repeatItems: '1x Zinger Deluxe Burger, 1x Fresh Lime Soda'
    }
  ];
  ...
  return (
    <div className="pos-layout" onClick={() => showMoreMenu && setShowMoreMenu(false)}>
            ...
            <div>
              {cashier ? (
                <button
                  onClick={() => { 
                    if (!isCashedOut) {
                      setToast({ message: 'Cash Out First', type: 'error' });
                    } else {
                      setCashier(null); 
            ...
            <div className="product-grid">
              {products.map(prod => (
                ...
                >
                  ...
                  <div className="product-desc">{prod.desc || 'Delicious and freshly prepared'}</div>
                  <div className="product-price-badge">Rs. {prod.price}</div>
            ...
            <div className="custom-scrollbar" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px', overflowY: 'auto', paddingRight: '10px' }}>
              ...
              {allOnlineOrders.map(order => (
                <div key={order.id} style={{ display: 'flex', flexDirection: 'column', gap: '15px', background: 'var(--bg-base)', padding: '20px', borderRadius: 'var(--radius-md)', borderLeft: `5px solid var(--primary)` }}>
                  ...
                  <div style={{ color: 'var(--text-muted)', fontSize: '0.9rem', flex: 1 }}>Items: {order.items}</div>
                          ...
                          {del.status === 'READY_FOR_RIDER' ? (
                            ...
                            <>
                              <div style={{ flex: 1 }}>
                                ...
                                <span className="delivery-settlement-amount">Rs. {del.cod}</span>
                              </div>
        ...
        <div className="modal-overlay">
          <div className="modal-content animate-slide-up" style={{ width: '400px' }}>
            ...
            <div style={{ padding: '20px' }}>
              ...
              <button className="btn-action btn-save" style={{ width: '100%', padding: '15px', fontSize: '1.1rem' }} onClick={() => {
                 const amt = parseFloat((document.getElementById('cashOutInput') as HTMLInputElement)?.value || '0');
                 if (amt > 0) {
                   const histStr = localStorage.getItem('d4u_cashin_history');
                   if (histStr) {
                     const hist = JSON.parse(histStr);
                     if (hist.length > 0) {
                       hist[0].cashOutAmount = amt;
                       hist[0].cashOutAt = new Date().toLocaleString();
                       localStorage.setItem('d4u_cashin_history', JSON.stringify(hist));
                     }
                   }
                   setPrintData({ type: 'BILL', data: { orderType: 'Cash Out Receipt', cashOutAmount: amt, time: new Date().toLocaleString() }, printCount: 1 });
              ...
              <div style={{ display: 'flex', gap: '10px', marginTop: '15px' }}>
                ...
                <button className="btn-action" onClick={async () => {
                  ...
                  try {
                    ...
                    const nextId = count > 0 ? (await db.products.toArray()).reduce((max, p) => p.id > max ? p.id : max, 0) + 1 : 1000;
                    await db.products.add({ id: nextId, category_id: customItemCategory || (categories[0]?.id || 1), name: customItemName, price: parseFloat(customItemPrice) || 0, img: 'https://images.unsplash.com/photo-1568901346375-23c9450c58cd?auto=format&fit=crop&w=300&q=80', desc: 'POS Custom Added Item', isApproved: false, itemCode: customItemCode });
                    setToast({ message: 'Custom Product Created (Pending Approval)!', type: 'success' });
      ...
      {/* GLOBAL TOAST */}
      {toast && (
        <div className="blink-animation" style={{ position: 'fixed', top: '30px', left: '50%', transform: 'translateX(-50%)', background: toast.type === 'success' ? 'var(--accent-green)' : 'var(--primary)', color: toast.type === 'success' ? 'black' : 'white', padding: '15px 25px', borderRadius: 'var(--radius-md)', boxShadow: '0 10px 25px rgba(0,0,0,0.3)', display: 'flex', alignItems: 'center', gap: '12px', zIndex: 9999, fontWeight: 'bold', fontSize: '1.1rem' }}>
          <CheckCircle size={24} />
          {toast.message}
        </div>
      )}
...
function DayStartPage({ currentUser, onDayStart }: { currentUser: typeof USERS[0]; onDayStart: (t: Date) => void }) {
  const [history] = useState<DayRecord[]>(() => {
    try { return JSON.parse(localStorage.getItem('d4u_day_history') || '[]'); } catch { return []; }
  });
  ...
  const handleStart = () => {
    ...
    const newRecord: DayRecord = {
      id: (lastRecord?.id ?? 600) + 1,
      dayStart: now.toISOString(),
      dayClose: '',
    };
    const updated = [...history, newRecord];
    localStorage.setItem('d4u_day_history', JSON.stringify(updated));
    onDayStart(now);
  };
  ...
  return (
    <div style={s.page}>
      ...
      <div style={s.left}>
        ...
        <button style={s.startBtn} onClick={handleStart}>Day Start</button>
      </div>
          ...
          <tbody>
            {demoRows.map((row) => (
              <tr key={row.id}>
                ...
                <td style={s.td}>{fmt(row.dayStart)}</td>
                <td style={s.td}>{row.dayClose ? fmt(row.dayClose) : <em style={{ color: '#9ca3af' }}>Active</em>}</td>
                <td style={s.td}>
...
function CashInPage({ currentUser, onCashIn }: { currentUser: typeof USERS[0]; onCashIn: (amount: number) => void }) {
  const [amount, setAmount] = useState('');
  const [comment, setComment] = useState('');
  const [history, setHistory] = useState<any[]>(() => {
    try { return JSON.parse(localStorage.getItem('d4u_cashin_history') || '[]'); } catch { return []; }
  });
  ...
  const handleCashIn = () => {
    ...
    const newRecord = {
      ...
      cashInAt: now.toLocaleString(),
      cashInAmount: parseFloat(amount),
      cashOutAt: '',
      cashOutAmount: 0,
      comment: comment
    };
    ...
    setHistory(updated);
    localStorage.setItem('d4u_cashin_history', JSON.stringify(updated));
  ...
  const demoRows = history.length > 0 ? history : [
    { id: 1277, cashInAt: '25/02/2020 3:28:43 PM', cashInAmount: 0, cashOutAt: '02/25/2020 3:28:48 PM', cashOutAmount: 0 },
    { id: 1146, cashInAt: '18/02/2020 3:16:03 PM', cashInAmount: 10000, cashOutAt: '02/18/2020 6:48:17 PM', cashOutAmount: 0 },
  ];
  ...
  return (
    <div style={s.page}>
      <div style={s.left}>
        ...
        <button style={s.startBtn} onClick={handleCashIn}>Cash in</button>
        ...
        <table style={s.table}>
          ...
          <tbody>
            {demoRows.map((row) => (
              <tr key={row.id}>
                <td style={s.td}>{row.id}</td>
                <td style={s.td}>{row.cashInAt}</td>
                <td style={s.td}>{row.cashInAmount}</td>
                <td style={s.td}>{row.cashOutAt || '-'}</td>
                <td style={s.td}>{row.cashOutAmount}</td>
                <td style={s.td}>
...
export default function App() {
  const [loggedInUser, setLoggedInUser] = useState<typeof USERS[0] | null>(() => {
    try { return JSON.parse(localStorage.getItem('d4u_main_user') || 'null'); } catch { return null; }
  });
  ...
  useEffect(() => {
    if (loggedInUser) localStorage.setItem('d4u_main_user', JSON.stringify(loggedInUser));
    else localStorage.removeItem('d4u_main_user');
  }, [loggedInUser]);
  ...
  if (!dayStartTime) return <DayStartPage currentUser={loggedInUser} onDayStart={setDayStartTime} />;
  if (!isCashedIn) return <CashInPage currentUser={loggedInUser} onCashIn={(amount) => { setCashInAmount(amount); setIsCashedIn(true); }} />;
  return <POSApp currentUser={loggedInUser} dayStartTime={dayStartTime} />;
}
